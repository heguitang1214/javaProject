...
public class RegxUtil {
     ...
     * </p>
     */
    public static boolean checkLoginAccount(String loginAccount) {
        if (null == loginAccount) {
            return false;
        }
        boolean result = false;
        if (loginAccount.matches("[A-Za-z]{6,20}$")) {
            result = true;
        } else if (loginAccount.matches("^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,20}$")) {
            result = true;
        }
        return result;
    }
    ...
    public static boolean checkSupplierMobile(String mobile) {
        if (null == mobile) {
            return false;
        }
        // 允许 + - 数字 空格
        return Pattern.matches("[0-9+\\-\\s]*\\{1,20\\}", mobile);
    }
