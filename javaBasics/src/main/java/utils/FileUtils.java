	...
	public static String downFile(File file, HttpServletRequest request, HttpServletResponse response, String fileName){
		...
		try {
			response.addHeader("Content-Disposition", "attachment; filename=\"" + 
					Encodes.urlEncode(StringUtils1.isBlank(fileName) ? file.getName() : fileName) + "\"");
...
//		String p = StringUtils11.replace(path, "\\", "/");
//		p = StringUtils1.join(StringUtils1.split(p, "/"), "/");
//		if (!StringUtils1.startsWithAny(p, "/") && StringUtils1.startsWithAny(path, "\\", "/")){
//			p += "/";
//		}
//		if (!StringUtils1.endsWithAny(p, "/") && StringUtils1.endsWithAny(path, "\\", "/")){
//			p = p + "/";
//		}
...
//	}
	...
	public static String getFileExtension(String fileName) {
		...
		return StringUtils1.lowerCase(fileName.substring(fileName.lastIndexOf(".") + 1));
	}
